...
class QuestRewardEngine {
  ...
  static EXP_PER_LEVEL = 1000;
  static DAILY_QUEST_STAT_CAP = 70;
  static STAT_ORDER_FIRST_EVOLUTION = ['health', 'diligence', 'focus', 'social', 'creativity'];
  static LINEAGE_TO_DOMINANT_STAT = {
    fire: 'health',
    water: 'diligence',
    sprout: 'focus',
    cloud: 'social',
  ...
  constructor(kstService = kstDateService) {
    this._kst = kstService;
  }

  maxStatForLevel(level) {
  ...
  async ensureStatsRow(conn, userId) {
    await conn.query(
  ...
  async _tryPetEvolutionOnce(conn, userId) {
    ...
    if (ev === 0 && animal === 'egg' && petLevel >= 6) {
      for (const statName of QuestRewardEngine.STAT_ORDER_FIRST_EVOLUTION) {
    ...
    if (ev === 1 && petLevel >= 11 && pet.lineage_type) {
      const domStat = QuestRewardEngine.LINEAGE_TO_DOMINANT_STAT[String(pet.lineage_type)] || null;
  ...
  async _tryPetEvolution(conn, userId) {
    let evolved = false;
    for (let i = 0; i < 2; i += 1) {
      const once = await this._tryPetEvolutionOnce(conn, userId);
      if (!once) break;
      evolved = true;
    }
    return evolved;
   ...
   */
  async applyQuestReward(conn, userId, quest) {
    await this.ensureStatsRow(conn, userId);

    const kstToday = this._kst.kstYmd();
    const rewardExp = Number(quest.reward_exp) || 0;
    const rewardCoin = this.isLlmQuest(quest) ? 0 : Number(quest.reward_coin) || 0;
    ...
    let levelUps = 0;
    while (exp >= QuestRewardEngine.EXP_PER_LEVEL) {
      exp -= QuestRewardEngine.EXP_PER_LEVEL;
      petLevel += 1;
      levelUps += 1;
    }
    ...
    const maxStat = this.maxStatForLevel(petLevel);
    let appliedStat = 0;
    if (QuestRewardEngine.ALLOWED_STATS.includes(statType) && statAmountRaw > 0) {
      const currentStat = Number(st[statType]) || 0;
      const roomCap = Math.max(0, maxStat - currentStat);
      const roomDaily = Math.max(0, QuestRewardEngine.DAILY_QUEST_STAT_CAP - questDailySum);
    ...
    const evolved = await this._tryPetEvolution(conn, userId);
...
const questRewardEngine = new QuestRewardEngine();

module.exports = {
  QuestRewardEngine,
  questRewardEngine,
  applyQuestReward: questRewardEngine.applyQuestReward.bind(questRewardEngine),
  ensureStatsRow: questRewardEngine.ensureStatsRow.bind(questRewardEngine),
  maxStatForLevel: questRewardEngine.maxStatForLevel.bind(questRewardEngine),
  isLlmQuest: questRewardEngine.isLlmQuest.bind(questRewardEngine),
  ALLOWED_STATS: QuestRewardEngine.ALLOWED_STATS,
  EXP_PER_LEVEL: QuestRewardEngine.EXP_PER_LEVEL,
  DAILY_QUEST_STAT_CAP: QuestRewardEngine.DAILY_QUEST_STAT_CAP,
};
